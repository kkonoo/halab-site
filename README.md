...
- 기본 **최근 3개월** 목록은 방문일 기준 3개월 전 같은 날짜 이후의 주차만 표시합니다.
  이전 주차는 **Archive / 보관함**에서 **연도 → 분기 → 주차** 순서로 선택합니다.
  자료가 있는 연도와 분기만 최신순으로 표시하며, 선택한 주차에서 분야 필터·키워드 검색을 지원합니다.
  이는 화면 표시 구분이며 데이터를 삭제하지 않습니다. 게시 데이터 생성 시 과거 주차도
  계속 포함해야 합니다 (`wiki/cards/` 원본과 전체 `cards-data.js` 보존).
